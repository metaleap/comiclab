...
func Main() {
	...
	if State.Config, err = readJSONFile(State.Config.FilePath(), &Config{}); err != nil {
		if !os.IsNotExist(err) {
			...
			State.Config = &Config{}
			State.Config.ContentAuthoring.Authors, State.Config.ContentAuthoring.Languages, State.Config.ContentAuthoring.ContentFields = map[string]string{}, map[string]string{}, map[string]bool{}
