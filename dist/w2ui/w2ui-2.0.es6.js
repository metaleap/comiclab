    ...
    message(where, options) {
        ...
        options.action = (action, event) => {
            ...
            let edata = options.trigger('action', {
                target: this.name, action, self: options,
    ...
    menuClick(overlay, event, index, parentIndex) {
        ...
        if (query(event.target).hasClass('remove')) {
            edata = this.trigger('remove', {
                originalEvent: event, target: overlay.name,
...
class w2toolbar extends w2base {
    ...
    insert(id, items, skipRefresh) {
        ...
        items.forEach((item, idx, arr) => {
            ...
            // checks
            let valid = ['label', 'button', 'check', 'radio', 'drop', 'menu', 'menu-radio', 'menu-check', 'color', 'text-color', 'html',
                    ...
                    setTimeout(() => {
                        ...
                        if (['menu', 'menu-radio', 'menu-check'].includes(it.type)) {
                            ...
                            w2menu.show(w2utils.extend({
                                ...
                                .remove(event => {
                                    this.menuClick({
                                        name: this.name, remove: true, item: it, subItem: event.detail.item,
                                        originalEvent: event
                                    })
                                })
                                .select(event => {
                                    this.menuClick({
                                        name: this.name, item: it, subItem: event.detail.item,
                                        originalEvent: event
                                    })
                                })
                        }
    ...
    render(box) {
        ...
        function mouseMove(evnt) {
            ...
            let edata = self.trigger('resizing', {
                target: self.name, object: panel, originalEvent: evnt,
...
class w2grid extends w2base {
    constructor(options) {
        ...
        this.buttons = {
            ...
            'columns': {
                type: 'menu-check', id: 'w2ui-column-on-off', icon: 'w2ui-icon-columns', tooltip: 'Show/hide columns',
                overlay: { align: 'none' }
            },
            'search': {
    ...
    request(action, postData, url, callBack) {
        ...
        if (action == 'load') {
            edata = this.trigger('request', {
                target: this.name, url, postData: params, httpMethod: 'GET',
