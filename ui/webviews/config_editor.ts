...
let grid_contentfields = ctl_inputgrid.create('config_contentfields', [
    ...
    { id: 'localizable', title: "Multi-Language", validators: [ctl_form.validatorLookup], lookUp: ctl_form.lookupBool },
], (userModifiedRecs) => {
    setDisabled(true)
...
let grid_paperformats = ctl_inputgrid.create('config_paperformats', [
    ...
    { id: 'widthMm', title: 'Width (mm)', num: { min: 11, max: 1234 }, validators: [ctl_form.validatorNonEmpty] },
    { id: 'heightMm', title: 'Height (mm)', num: { min: 11, max: 1234 }, validators: [ctl_form.validatorNonEmpty] },
], (userModifiedRecs) => {
    setDisabled(true)
...
function newGridForStringMap(id: string, title: string, valueName: string, valueTitle: string, cur: () => ctl_form.Rec[], set: (_: { [_: string]: string }) => void) {
    return ctl_inputgrid.create(id, [
        { id: 'id', title: title + " ID", validators: [/*validators added by input_grid.create*/] },
        { id: valueName, title: valueTitle, validators: [ctl_form.validatorNonEmpty, ctl_inputgrid.validatorUnique(cur)] },
    ], (userModifiedRecs) => {
        setDisabled(true)
